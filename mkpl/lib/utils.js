...
export const useMappedReducer = (reducers, initState, stateTypes) => {
  const ActionTypes = useMemo(() => createEnum(...Object.keys(reducers)), [
    reducers
  ]);
  return [
...
export const toQueryParams = ({ search, options }) => {
  ...
  if (value.length !== 0) {
    const effectiveOptions = filter(
      options,
      op =>
        op.operator === Operators.MATCH ||
        (op.operator === Operators.EQ && value.match(/^\d+$/))
    );
...
export const numberFormatter = (val, locale) =>
  Numbro.culture(locale)(val).format('0,0.00');
export const integerFormatter = (val, locale) =>
  Numbro.culture(locale)(val).format('0,0');
